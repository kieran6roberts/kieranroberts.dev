...
export const Button = React.forwardRef<HTMLButtonElement | HTMLAnchorElement, ComponentType>(
  (
    ...
    const { styleToApplyOnBoop: iconStyleToApply, handleBoopTrigger: handleIconTrigger } = useBoop(
      iconAnimationSettings || {}
    );
    const { styleToApplyOnBoop: buttonStyleToApply, handleBoopTrigger: handleButtonTrigger } =
    ...
    if (asLink && href) {
      return (
        <a
          ...
          href={href}
          className={cn(
            'flex items-center gap-2 text-sm lg:text-base font-medium rounded-full transition-colors duration-100 focus:ring outline-none ring-offset-2 px-3 py-2 lg:px-4 lg:py-2',
            className ?? '',
    ...
    return (
      <animated.button
        ...
        disabled={disabled}
        className={cn(
          'flex items-center justify-center gap-2 text-sm lg:text-base font-medium transition-colors duration-100 rounded-full focus:ring outline-none ring-offset-2 px-3 py-2 lg:px-4 lg:py-2',
          className ?? '',
          buttonVariants[variant],
          buttonWidth[width]
        )}
