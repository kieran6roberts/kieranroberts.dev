import { BlogPostsDocument, PostPagePostsDocument, type PublicationPostConnection, type PageInfo, type PostEdge, type Post } from "generated/graphql";
...
export const getAllPostPagePosts = async () => {
 ...
 while (hasNext) {
   ...
   if (blogPostData) {
     const { cursor, hasNextPage, postsArray } = getPostsPaginatedData(blogPostData as any);
...
export const getPostsPaginatedData = (blogPosts: PublicationPostConnection[] & { pageInfo: PageInfo; edges: [PostEdge] }) => {
 return ({
